...
var PPKLIB = function() {
    ...
    //ODIN根标识解析服务
    const PPK_ROOT_ODIN_API_URL = "https://ppk001.sinaapp.com/ppkapi2router/";
    ...
    var _readCache = function(uri){
        try{
            str_cache_info = getLocalConfigData( uri , true );
            ...
            if( exp_utc!=-1 && exp_utc < now){
                ...
                removeLocalConfigData(uri, true );
                return null;
            }
            
            chunk_content_bytes = getLocalConfigData(obj_cache_info.data_key, true );
    ...
    var _saveCache = function(ppk_uri,str_pttp_data){
      try{
        ...
        saveLocalConfigData( ap_resp_ppk_uri, str_cache_info , true );
        saveLocalConfigData( obj_cache_info.data_key, chunk_content_bytes , true ); 
    ...
    var _getApDataByAjax = function(obj_ppk_uri_info,path_level,parent_odin_setting,use_cache,callback_function){
        ...
        if(path_level==0){ //根标识
            ...
            str_ap_url = PPK_ROOT_ODIN_API_URL+"?pttp="+encodeURIComponent(current_uri);
    ...
    return {
        ...
        //参数prior_add_resource_mark_for_id取值true时 优先追加资源标识符（主要用于ID使用时）， 否则根据常用网址规则自动判断添加缺少的"/"字符和资源标志（用于访问网页内容）
        //注：特殊兼容DID的用户标识(did:ppk:)的格式化处理，得到以ppk:起始的URI
        formatPPkURI: function(user_uri,prior_add_resource_mark_for_id){ 
          try{
            //alert("formatPPkURI:"+user_uri);

            if(typeof user_uri == "undefined" || user_uri==null || user_uri.length==0)
                return null;
            
            prior_add_resource_mark_for_id = typeof prior_add_resource_mark_for_id !== 'undefined' ? prior_add_resource_mark_for_id:true;
            ...
            user_uri=user_uri.replace("：",":"); //替换容易输错的字符
        ...
        //调用AJAX方式获取PPK网络资源
        loadPPkDataByAjax: function(ppk_uri,use_cache,callback){
        ...
        //调用Plugin方式获取PPK网络资源
        loadPPkDataByPlugin: function(ppk_uri,use_cache,callback){
        ...
        getPPkData: function(ppk_uri,callback,use_cache){
        ...
        deleteCache: function(uri){
          try{
            if(_mbSupportPeerWebPlugin){
                console.log("Delete plugin cache for "+uri);
                PeerWeb.deleteNetCache(uri);
            }else{
                console.log("Delete h5 cache for "+uri);
                removeLocalConfigData(uri, true );
            }
          }catch(e){
            console.log("deleteCache() error:"+e);
          }
        },
